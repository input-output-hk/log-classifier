...
connDataLayer :: forall m. (MonadIO m, MonadCatch m, MonadMask m, MonadReader Config m) => DataLayer m
...
-- We need to sync occasionaly.
connPoolDataLayer
    :: forall m. (MonadBaseControl IO m, MonadIO m, MonadCatch m, MonadMask m, MonadReader Config m)
    => DBConnPool
    -> DataLayer m
