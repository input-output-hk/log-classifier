...
data HTTPNetworkLayer = HTTPNetworkLayer
    { hnlAddJsonBody    :: forall m a.  (ToJSON a, MonadThrow m) => a -> Request -> m Request
    ...
    , hnlApiCallSafe    :: forall m a.  (MonadIO m, MonadThrow m, FromJSON a) => (Value -> Parser a) -> Request -> m a
    }
